...
def visualize_result(data, pred, dir_result ,grd_dir_result):
    (img, seg, info) = data
    ...
    grd_im_vis = seg_color.astype(np.uint8)

    img_name = info.split('/')[-1]
    Image.fromarray(im_vis).save(os.path.join(dir_result, img_name.replace('.jpg', '.png')))
    Image.fromarray(grd_im_vis).save(os.path.join(grd_dir_result, img_name.replace('.jpg', '.png')))
    ...
    for batch_data in loader:
        ...
        if cfg.VAL.visualize:
            visualize_result(
                ...
                os.path.join(cfg.DIR, 'grd_result')
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--cfg",
        # default="model_two_apartment_0/config.yaml",
        default="model_two_others/config.yaml",
        metavar="FILE",
    ...
    if not os.path.isdir(os.path.join(cfg.DIR, "grd_result")):
        os.makedirs(os.path.join(cfg.DIR, "grd_result"))
