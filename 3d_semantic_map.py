...
### custom voxel down ###
def custom_voxel_down(pcd_final):
        ...
        for j in range(int(y_min*100), int(y_max*100), int(voxel*100)):
            for k in range(int(z_min*100), int(z_max*100), int(voxel*100)):
                ...
                if counts != []:
                    ...
                    point.append([(i/100)+voxel/2, (j/100)+(voxel)/2, (k/100)+(voxel)/2])
    ...
    o3d.io.write_point_cloud('final.pcd',pcd_custom)
...
DIR = './reconstuct_data_one/one_others_result' #要統計的資料夾
NUMBER_IMG=10
...
while(NUMBER_IMG>0):
    if(use_img==0):
        ...
        img0=cv2.imread('./reconstuct_data_one/one_others_result/'+'images_'+str(num_img)+'.png',1)
        #color#
        img1=cv2.imread('./reconstuct_data_one/depth/'+'depth_'+str(num_img)+'.png',1)
        temp=(img0,img1)
        all_img.append(temp)
