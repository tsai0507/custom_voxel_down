...
def navigateAndSee(action=""):
    if action in action_names:
        ...
        #回傳front_rgb,front_depth,front_semantic_label座標
...
#相機座標每移動一次就將存下front_rgb,depth_rgb,front_semantic_label
def save_reconstruct(save_img,count):
    #用count來命名照片,才能有多張照片資料
    cv2.imwrite('./reconstuct_data/images/'+'images_'+str(count)+'.png',save_img[0])
    cv2.imwrite('./reconstuct_data/depth/'+'depth_'+str(count)+'.png',save_img[1])
    cv2.imwrite('./reconstuct_data/annotations/'+'annotations_'+str(count)+'.png',save_img[2])         
count=1                                         #幫照片編號
